...
const DonorDashboard = () => {
  ...
  // Fetch scholarships in Live Mode
  const { data: scholarships, loading: scholarshipsLoading } = useScholarships({ page: 1, limit: 100 })
  ...
  // Convert scholarships to student format for Live Mode
  const liveStudents = isDemo ? [] : (scholarships || []).map(s => ({
    id: s.id,
    wallet: s.student_wallet || '0x...',
    dream: s.title,
  ...
  // Demo Mode: Use mockStudents, Live Mode: Use liveStudents from Supabase
  const studentsToFilter = isDemo ? mockStudents : liveStudents
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-sm text-amber-200">
                Connect your wallet to make donations and track your impact. Browsing students is available without a wallet.
              </p>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-sm text-indigo-200">
                Demo mode: Browse students freely. Connect wallet to make donations.
              </p>
            </div>
          </motion.div>
        )}
        ...
        >
          <div className="card text-center">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">
              {isDemo ? mockStudents.length : liveStudents.length}
            </div>
          ...
          <div className="card text-center">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">
              {isDemo 
                ...
                : new Set(liveStudents.map(s => s.country)).size}
            </div>
          ...
          <div className="card text-center">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">
              {isDemo 
                ...
                : liveStudents.filter(s => s.verifiedByInstitution).length}
            </div>
          ...
          <div className="card text-center">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">
              ₹{isDemo 
                ? (mockStudents.reduce((sum, s) => sum + (s?.totalFunded || 0), 0) || 0).toLocaleString()
                : (liveStudents.reduce((sum, s) => sum + (s?.totalFunded || 0), 0) || 0).toLocaleString()}
            </div>
